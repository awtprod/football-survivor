  ...
  for (const g of espnGames) {
    ...
    for (const side of ['home', 'away']) {
      ...
      rows.push({
        ...
        status: g.status, statusDetail: g.statusDetail, score: g.homeScore != null && g.status !== 'pre' ? `${g.awayScore}-${g.homeScore}` : null,
...
/** Which entries are still alive as of `week` (weeks before it that have results). An entry with no recorded pick in a finished week is treated as eliminated. */
export function aliveEntries(entries, projection, week) {
  const won = {}; // week -> team -> true/false (only finished games)
  for (const [team, arr] of Object.entries(projection)) for (const x of arr) if (x.done) (won[x.week] ??= {})[team] = x.won;
  // A finished week with no picks recorded at all means the sheet predates it: unknown, not an elimination.
  const recorded = new Set(entries.flatMap((e) => Object.keys(e.picks).map(Number)));
  return entries.map((e) => {
    let out = null;
    for (let w = 1; w < week; w++) {
      if (!won[w] || !recorded.has(w)) continue;
      const t = e.picks[w];
      if (!t) { out = { week: w, team: null }; break; }
      if (won[w][t] === false) { out = { week: w, team: t }; break; }
    }
