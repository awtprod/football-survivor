...
export function rateWeek({ espnGames, nvGames, elo, injuries, season, week, used, remainingWeeks }) {
  ...
  for (const g of espnGames) {
    ...
    for (const side of ['home', 'away']) {
      ...
      rows.push({
        ...
        status: g.status, statusDetail: g.statusDetail, done: g.status === 'post', score: g.homeScore != null && g.status !== 'pre' ? `${g.awayScore}-${g.homeScore}` : null,
  ...
  for (const r of rows) {
    ...
    r.survivor = r.prob - oppCost - (r.used ? 1 : 0);
    r.flags = [];
  ...
  return rows.sort((a, b) => b.survivor - a.survivor);
}
...
export function projectSeason({ nvGames, elo, season }) {
  ...
  for (const g of nvGames) {
    ...
    (out[g.home] ??= []).push({ week: g.week, opp: g.away, home: true, prob: pHome, done: g.result != null, won: g.result != null ? g.result > 0 : null, tie: g.result === 0 });
    (out[g.away] ??= []).push({ week: g.week, opp: g.home, home: false, prob: 1 - pHome, done: g.result != null, won: g.result != null ? g.result < 0 : null, tie: g.result === 0 });
  }
  return out;
}
...
/* ---------- Pool (other entries' history) ---------- */
const CROWD_K_DEFAULT = 12; // softmax steepness over win prob; fitted from this pool's past weeks when available

/** Per-week finished results and whether every scheduled game that week is final. Derived from a
 *  season projection. A team on bye has no entry that week, so it never blocks the all-final check. */
export function weekResults(projection) {
  const won = {}, tie = {}, complete = {}; // week -> team -> bool ; week -> all games final?
  for (const [team, arr] of Object.entries(projection)) for (const x of arr) {
    complete[x.week] = (complete[x.week] ?? true) && x.done;
    if (x.done) { (won[x.week] ??= {})[team] = x.won; if (x.tie) (tie[x.week] ??= {})[team] = true; }
  }
  return { won, tie, complete };
}

/**
 * Which entries are still alive through `week` (inclusive, so finished games in the current week grade too).
 * An entry is eliminated the moment a recorded pick loses a final game. A blank pick eliminates only once its
 * whole week is final — a partial week (some games played, some pending) leaves blanks alive, so an
 * incompletely-entered sheet does not wrongly wipe everyone. A week with no picks recorded pool-wide is treated
 * as predating the sheet (unknown), and ties survive (a tie is not a loss).
 */
export function aliveEntries(entries, projection, week) {
  const { won, tie, complete } = weekResults(projection);
  const recorded = new Set(entries.flatMap((e) => Object.keys(e.picks).map(Number)));
  return entries.map((e) => {
    let out = null;
    for (let w = 1; w <= week; w++) {
      if (!recorded.has(w)) continue; // sheet predates this week: unknown, not an elimination
      const t = e.picks[w];
      if (t) {
        if (won[w]?.[t] === false && !tie[w]?.[t]) { out = { week: w, team: t }; break; } // recorded loss -> out now
        // won, tie, or game not yet final (pending) -> survives this week
      } else if (complete[w]) { out = { week: w, team: null }; break; } // blank -> out only once the week is final
    }
    return { ...e, alive: !out, out };
  });
}

/** Fit crowd steepness k by maximum likelihood over past weeks' actual picks vs. that week's projected win probs. */
export function fitCrowdK(entries, projection, week) {
  const probs = {}; for (const [team, arr] of Object.entries(projection)) for (const x of arr) (probs[x.week] ??= {})[team] = x.prob;
  const obs = []; // [{avail: {team: p}, pick}]
  for (const e of entries) for (let w = 1; w < week; w++) {
    const t = e.picks[w]; if (!t || !probs[w]?.[t]) continue;
    const used = new Set(Object.entries(e.picks).filter(([ww]) => +ww < w).map(([, x]) => x));
    const avail = Object.fromEntries(Object.entries(probs[w]).filter(([team]) => !used.has(team)));
    obs.push({ avail, pick: t });
  }
  if (obs.length < 20) return { k: CROWD_K_DEFAULT, n: obs.length, fitted: false };
  let best = { k: CROWD_K_DEFAULT, ll: -Infinity };
  for (let k = 2; k <= 30; k += 1) {
    let ll = 0;
    for (const o of obs) { let z = 0; for (const p of Object.values(o.avail)) z += Math.exp(k * p); ll += k * o.avail[o.pick] - Math.log(z); }
    if (ll > best.ll) best = { k, ll };
  }
  return { k: best.k, n: obs.length, fitted: true };
}

/**
 * Forecast this week's pick distribution among alive entries and the leverage of each candidate.
 * leverage = (expected surviving fraction if an average team wins) / (expected surviving fraction if THIS team wins).
 * >1 means the crowd is elsewhere (a win thins the field more than usual); <1 means you are riding with the crowd.
 */
export function poolAnalysis({ entries, projection, week, rows, k = CROWD_K_DEFAULT }) {
  const live = aliveEntries(entries, projection, week);
  const alive = live.filter((e) => e.alive);
  const prob = {}; const oppOf = {};
  for (const r of rows) { prob[r.team] = r.prob; oppOf[r.team] = r.opp; }
  const teams = Object.keys(prob);
  const share = Object.fromEntries(teams.map((t) => [t, 0]));
  for (const e of alive) {
    const used = new Set(Object.values(e.picks));
    const avail = teams.filter((t) => !used.has(t)); if (!avail.length) continue;
    let z = 0; const w = {}; for (const t of avail) { w[t] = Math.exp(k * prob[t]); z += w[t]; }
    for (const t of avail) share[t] += w[t] / z / alive.length;
  }
  // Expected surviving fraction if team t wins: t's crowd survives, t's opponent's crowd dies, others survive at their own prob.
  const survIf = {};
  for (const t of teams) { let s = 0; for (const u of teams) s += u === t ? share[u] : u === oppOf[t] ? 0 : share[u] * prob[u]; survIf[t] = s; }
  const base = teams.reduce((s, t) => s + survIf[t] * prob[t], 0) / Math.max(1e-9, teams.reduce((s, t) => s + prob[t], 0));
  const leverage = Object.fromEntries(teams.map((t) => [t, clamp(base / Math.max(survIf[t], 1e-6), 0.6, 1.6)]));
  // Actual distributions for past weeks (what the sheet recorded), for display.
  const history = {};
  for (const e of entries) for (const [w, t] of Object.entries(e.picks)) { const h = (history[w] ??= { n: 0, teams: {} }); h.n++; h.teams[t] = (h.teams[t] || 0) + 1; }
  return { total: entries.length, paid: entries.filter((e) => e.paid).length, alive: alive.length, k, share, leverage, survIf, history,
    eliminated: live.filter((e) => !e.alive).length,
    entries: live.map((e) => ({ name: e.name, alive: e.alive, out: e.out, picks: e.picks })) };
}
