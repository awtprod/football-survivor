...
//
//   users[uid] = { sub, email, emailLower, name, picture, role, createdAt, lastSeenAt,
...
export const DEFAULT_LEAGUE_SETTINGS = () => ({
  chalkFactor: 1, myEntry: '', myEntries: [], lambda: 1, mustDiffer: false, behaviour: false, elite: [], entrantChalk: {},
});
...
function load() {
  ...
  state = migrateV1(raw, process.env.ADMIN_EMAIL || '');
 ...
 */
export function upsertUser({ sub, email, name, picture, normalize }) {
  const uid = `g:${sub}`;
  ...
  save((s) => {
    ...
    const now = new Date().toISOString();
    s.users[uid] ||= {
      sub, email, emailLower: lower, name: name || '', picture: picture || '', role: 'member',
