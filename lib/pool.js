...
const FILE = path.join(DATA_DIR, 'pool.json');
...
function unzip(buf) {
  ...
  for (let n = 0; n < count; n++) {
    ...
    if (method === 0) out[name] = raw; else if (method === 8) out[name] = inflateRawSync(raw); else throw new Error(`unsupported zip method ${method}`);
...
export function load() { if (!existsSync(FILE)) return null; try { return JSON.parse(readFileSync(FILE, 'utf8')); } catch { return null; } }
export function save(parsed, meta = {}) {
  const doc = { ...parsed, importedAt: new Date().toISOString(), ...meta };
  writeFileSync(FILE + '.tmp', JSON.stringify(doc)); renameSync(FILE + '.tmp', FILE); return doc;
}
