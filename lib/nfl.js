...
export async function fetchText(url, ms = 30000) {
  const ctl = new AbortController();
  const t = setTimeout(() => ctl.abort(), ms);
  try {
...
export async function cached(name, ttlMs, loader) {
...
// Full RFC-4180-ish tokenizer: scans the whole text once so a newline or comma *inside* a quoted
// field, and an escaped "" quote, never break row alignment. (Splitting on '\n' first would.)
function parseCsv(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (q) {
      ...
      if (ch === '\r' && text[i + 1] === '\n') i++;
      row.push(cur); cur = ''; if (row.some((c) => c !== '')) rows.push(row); row = [];
    } else cur += ch;
  }
  row.push(cur); if (row.some((c) => c !== '')) rows.push(row);
  if (!rows.length) return [];
  const head = rows[0];
  return rows.slice(1).map((cells) => { const o = {}; head.forEach((h, i) => (o[h] = cells[i] ?? '')); return o; });
}
