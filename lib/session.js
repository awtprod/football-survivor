...
/** sid -> { uid, email, name, picture, createdAt, lastSeenAt, ua } */
...
export function create({ uid, email, name, picture, ua }) {
  const now = Date.now();
  ...
  sessions.set(sid, { uid, email, name: name || '', picture: picture || '', createdAt: now, lastSeenAt: now, ua: String(ua || '').slice(0, 200) });
  dirty = true;
  return sid;
}
