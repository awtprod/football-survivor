...
const planRows = await p.$$eval('#v-season table tr', (r) => r.length);
const heatCells = await p.$$eval('#v-season .heat .c', (r) => r.length);
ok('season view builds a plan table', planRows > 1);
ok('season view builds the heat grid', heatCells > 0);
// trends
await p.click('nav button[data-v="trends"]'); await shot('trends');
const paths0 = await p.$$eval('#trend path', (r) => r.length);
ok('trend chart draws at least one series line', paths0 > 0);
await p.click('#v-trends .chips button[data-t="KC"]'); await new Promise(r=>setTimeout(r,200));
const paths1 = await p.$$eval('#trend path', (r) => r.length);
ok('toggling a team on adds a trend line', paths1 > paths0);
...
ok('hovering the trend chart shows the tooltip', (await p.$eval('#tip', (e) => e.style.display)) !== 'none');
